...
ファイルは，ダウンロードしておき，適当なフォルダに保存しておく．
```{r input file}
...
x2 <- c("だみー", "ススキ", "ハリガネワラビ", "オミナエシ", "コナスビ", "カナビキソウ", 
  ...
  # 入力和名に対する和名・学名の候補を出力
wamei_check(x1, hub_master, jn_master)
wamei_check(x1, hub_master, jn_master, wide=FALSE)
wamei_check(x2, hub_master, jn_master,             ds=c(GL, SF, WF))
wamei_check(x2, hub_master, jn_master, wide=FALSE, ds=c(GL, SF, WF))

  # エクセル形式と同等の出力
wamei_check_ex(x1, hub_master, jn_master, wide=FALSE)
