...
#' Check the detail by Wamei checklist (ver. 1.10). 
#'   Yamanouchi, T., Shutoh, K., Osawa, T., Yonekura, K., Kato, S., Shiga, T.
#'     2019. A checklist of Japanese plant names 
#'     (\url{https://www.gbif.jp/v2/activities/wamei_checklist.html})
#' The variables are as follows:
#'
#' @format A data frame with 30430 rows and 12 variables: 
#' 各項目の説明は，\url{https://www.gbif.jp/v2/activities/wamei_checklist.html}から．
#' \describe{
#'   \item{all_name}{引用元に掲載された、異名を含むすべての和名。}
#'   \item{Hub name}{
#'   データベース上での標準的な和名を示す。原則として各データソースでの標準的な
...
#'   \item{status}{
#'   各データソース間で、和名の指す分類群の同一性が確認できたものは"確定"とし、
#'   和名の指す範囲が異なるなど分類学的な検討を要するものは"！未統合"とした。
#'   }
#'   \item{message}{
#'   一つのall nameに対し複数のHub nameが相当する、すなわち異物同名が存在する
